...
export class Game {
  ...
  // localStorage에서 클리어 진행도 로드
  loadProgress() {
    const saved = localStorage.getItem('loop_maze_progress');
    if (saved) {
      this.progress = JSON.parse(saved);
    } else {
      this.progress = { normal: 0, hard: 0 };
    }
  }

  // localStorage에 현재 진행도 저장
  saveProgress() {
    localStorage.setItem('loop_maze_progress', JSON.stringify(this.progress));
  }
  ...
  clearCurrentStage() {
    // 저장 데이터 업데이트
    if (this.currentStageIndex + 1 > this.progress[this.currentMode]) {
      this.progress[this.currentMode] = this.currentStageIndex + 1;
      this.saveProgress();
    }

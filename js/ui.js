export class UI {
  ...
  injectStyles() {
    ...
    style.innerHTML = `
      ...
      .glow-btn.hard:hover:not(:disabled) {
        background: #ff0055;
        color: #000;
  ...
  render() {
    ...
    container.innerHTML = `
      ...
      <div id="settings-menu" class="glow-box" style="display: none; pointer-events: auto; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); padding: 30px; text-align: center;">
        ...
        <button class="glow-btn hard" id="btn-reset-data" style="margin-bottom: 15px;">진행 데이터 초기화</button><br>
  ...
  bindEvents() {
    ...
    document.getElementById('btn-reset-data').addEventListener('click', () => {
      if (confirm('모든 진행 저장 기록을 초기화하시겠습니까?')) {
  ...
  openStageSelect(mode) {
    ...
    const maxUnlocked = this.game.progress[mode];

    for (let i = 0; i < 10; i++) {
      const btn = document.createElement('button');
      const isCleared = i < maxUnlocked;
      const isUnlocked = i <= maxUnlocked;

      btn.className = `glow-btn ${mode === 'hard' ? 'hard' : ''} ${isCleared ? 'cleared' : ''}`;
      btn.innerText = isCleared ? `${i + 1} ✓` : i + 1;
